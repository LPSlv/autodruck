...
function cycle(x: number[], k: number[]): void {
  let [a, b, c, d] = x;
...
function blk(bytes: number[], off: number): number[] {
...
function rhex(n: number): string {
  let s = '';
  for (let j = 0; j < 4; j++) s += hexc[(n>>(j*8+4))&0x0f] + hexc[(n>>(j*8))&0x0f];
  return s;
}

export function md5(input: string): string {
  const bytes = utf8(String(input));
